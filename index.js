        ...
        <div id="tab-summary" class="tab-content active">
            ...
            <div class="free-cancel-badge">
                ...
                <div>
                    ...
                    <div style="font-weight:400; font-size: 0.65rem; opacity: 0.8;">Fino a 7 giorni prima della partenza</div>
                </div>
            </div>
...
function prenota(idx) {
  ...
  const basePrice = Math.round(
    CONFIG.basePrices[idx] * (CONFIG.multipliers[month] || 1),
  );
  ...
  const dateStr = `${STATE.selectedDays[idx]} ${CONFIG.months[month]} 2024`;
  const boatName = card.querySelector("h3").innerText;

  // Salva nello stato globale per l'uso successivo (invio whatsapp)
  STATE.currentBookingData = { boatName, dateStr, finalPrice, selectedExtras };

  // 3. Generazione e Iniezione HTML Popup
  const summary = document.getElementById("modal-summary");
  const missingExtras = CONFIG.allExtras.filter(
    (item) => !selectedExtras.includes(item),
  );

  summary.innerHTML = getCheckoutTemplate({
    boatName,
    finalPrice,
    selectedExtras,
    missingExtras,
  ...
  // 4. Setup Event Listeners nel Popup (usiamo funzione unificata)
...
function setupBookingListeners() {
  ...
  if (!nome || !telefono || !check || !btn) return;

  // Funzione Validazione Unificata
  const validate = () => {
    const isValid =
    ...
    if (isValid) {
      ...
      btn.style.background =
        "linear-gradient(135deg, #0984e3 0%, #1e3799 100%)";
      btn.style.color = "#ffffff";
      btn.style.cursor = "pointer";
      btn.style.boxShadow = "0 10px 25px rgba(30, 55, 153, 0.4)";
      btn.style.transform = "scale(1.02)";
      btn.innerHTML =
        'Procedi al Pagamento <i class="fas fa-arrow-right" style="margin-left: 8px;"></i>';
  ...
  // Attach listeners
  nome.oninput = validate;
  telefono.oninput = validate;
  check.onchange = validate;

  // Attach Final Action
  btn.onclick = () => {
    if (!STATE.currentBookingData) return;

    const { boatName, dateStr, finalPrice, selectedExtras } =
      STATE.currentBookingData;
    const extrasStr =
      selectedExtras.length > 0
        ? `%0AExtra scelti: ${selectedExtras.join(", ")}`
        : "";
    const clientName = nome.value;

    const message = `*RICHIESTA PRENOTAZIONE*%0A%0A*Cliente:* ${clientName}%0A*Barca:* ${boatName}%0A*Giorno:* ${dateStr}%0A*Totale Previsto:* €${finalPrice}${extrasStr}`;
 ...
 * 6. INIZIALIZZAZIONE
 */
function forceTwemoji(context = document.body) {
  if (typeof twemoji !== "undefined")
    twemoji.parse(context, { folder: "svg", ext: ".svg" });
}
